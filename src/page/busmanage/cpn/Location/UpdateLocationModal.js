...
const UpdateLocationModal = ({ show, onHide, location, onUpdate }) => {
    ...
    const [previewImage, setPreviewImage] = useState(location ? location.location_imgUrl : '');

    useEffect(() => {
        setUpdatedLocation(location);
        setPreviewImage(location ? location.location_imgUrl : '');
    }, [location]);

    const handleChange = (e) => {
        const { name, value } = e.target;
        setUpdatedLocation({ ...updatedLocation, [name]: value });
        if (name === 'location_imgUrl') {
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setUpdatedLocation({ ...updatedLocation, location_imgUrl: reader.result });
        ...
        <Modal show={show} onHide={onHide}>
            ...
            <Modal.Body>
                <Form>
                    ...
                    <Form.Group controlId="formLocationImg">
                        <Form.Label>Image</Form.Label>
                        <Form.Control
                            type="file"
                            name="location_imgFile"
                            onChange={handleFileChange}
                        />
                        <Form.Control
                            type="text"
                            placeholder="Or enter image URL"
                            name="location_imgUrl"
                            value={updatedLocation.location_imgUrl || ''}
                            onChange={handleChange}
                            style={{ marginTop: '10px' }}
                        />
                        {previewImage && <img src={previewImage} alt="Image Preview" style={{ width: '100px', height: '100px', marginTop: '10px' }} />}
                    </Form.Group>
